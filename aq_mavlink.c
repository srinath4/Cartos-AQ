...
#ifdef USE_CARTOS
                case MAVLINK_MSG_ID_CUSTOM_TRAJECTORY:
                      if (mavlink_msg_custom_trajectory_get_target_system(&msg) == mavlink_system.sysid) {
                          uint16_t seqId;
                          uint8_t ack = 0;
                          uint8_t frame;
                          seqId = mavlink_msg_custom_trajectory_get_seq(&msg);
                          frame = mavlink_msg_custom_trajectory_get_frame(&msg);
                          if (seqId >= NAV_MAX_MISSION_LEGS || (frame != MAV_FRAME_GLOBAL && frame != MAV_FRAME_GLOBAL_RELATIVE_ALT)) {
                              // NACK
                              ack = 1;
                          }
                              ...
                              navMission_t *wp;
                              wp = navGetWaypoint(seqId);
                              wp->type = NAV_LEG_SPLINE;//This should not be hard coded as i want to use this msg for all custom types
                              wp->targetLat = mavlink_msg_custom_trajectory_get_x(&msg);
                              wp->targetLon = mavlink_msg_custom_trajectory_get_y(&msg);
                              wp->targetAlt = mavlink_msg_custom_trajectory_get_z(&msg);
                              wp->targetRadius = mavlink_msg_custom_trajectory_get_param1(&msg);
                              wp->loiterTime = mavlink_msg_custom_trajectory_get_param2(&msg) * 1000;
                              wp->maxHorizSpeed = mavlink_msg_custom_trajectory_get_param3(&msg);
                              wp->poiHeading = mavlink_msg_custom_trajectory_get_param4(&msg);
                              wp->sStart = mavlink_msg_custom_trajectory_get_sStart(&msg);
                              wp->sEnd = mavlink_msg_custom_trajectory_get_sEnd(&msg);
                              wp->y2N = mavlink_msg_custom_trajectory_get_y2N(&msg);
                              wp->y2E = mavlink_msg_custom_trajectory_get_y2E(&msg);
                              wp->y2D = mavlink_msg_custom_trajectory_get_y2D(&msg);
                              mavlinkData.wpCurrent = seqId + 1;
                              mavlinkData.wpAttempt = 0;
                              mavlinkData.wpNext = timerMicros();
                              ack = 0;
