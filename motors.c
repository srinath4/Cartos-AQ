...
/*#ifdef HAS_ONBOARD_ESC
    //This is the voltage to pwm conversion!
    value = analogData.vIn * pwm /MOTORS_SCALE;
#else*/
...
void motorsSendThrust(void) {
    float value;
#if defined(HAS_ONBOARD_ESC) || defined(USE_KISS_ESC)
    float nominalBatVolts, voltageFactor;
#endif
    int i;
...
void motorsCommands(float throtCommand, float pitchCommand, float rollCommand, float ruddCommand) {
   ...
   for (j = 0; j < MOTORS_NUM; j++)
	if (motorsData.active[j]){
		motorsData.thrust[j] = motorsPwm2Thrust(motorsData.value[j]*100);
	}
#endif
#endif
}
